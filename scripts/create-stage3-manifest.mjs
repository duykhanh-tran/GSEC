...
const outputPath = join(projectRoot, 'checkpoints', 'stage-3', 'manifest.json')
...
const files = [
  ...
  'vite.config.ts',
  'vitest.config.ts',
  'checkpoints/stage-3/validation.log',
]
...
const uniqueFiles = [...new Set(collected.map((path) => path.replaceAll('\\', '/')))].sort()
const manifestFiles = []
